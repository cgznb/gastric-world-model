...
十个种子的新旧模型全部完成：40个拟合阶段、830个实际epochs、14,110次优化器更新。
每种子521训练、65验证、65测试；40个选定检查点均早于全局冻结时间，全部测试产物晚于冻结。
各测试集有13个pCR阳性、14或15个记录复发阳性，十个测试集互不相同但两两重叠2至11人。
服务器记录的冻结时间为2026-09-16 16:45:24 UTC，最终独立审计通过于16:55:01 UTC。
本任务文档日期沿用2026-09-17，运行时间按服务器实际记录保留。

以下为V2相对同一种子V1的配对比较；计数不是独立显著性检验，差值范围不是置信区间。
不挑选最佳种子、不合并十个测试集，也没有在看测试结果后调整架构、训练或阈值。

| 指标 | 提高 / 下降 / 持平的种子数 | V2减V1的差值范围 |
| --- | ---: | ---: |
| pCR AUROC | 7 / 3 / 0 | -0.0888 至 +0.1420 |
| pCR AUPRC | 6 / 4 / 0 | -0.1024 至 +0.1354 |
| 记录复发 AUROC | 3 / 6 / 1 | -0.1625 至 +0.0813 |
| 记录复发 AUPRC | 3 / 7 / 0 | -0.1518 至 +0.1783 |

V2的pCR AUROC在9/10个种子低于普通与平衡logistic参考。
CT1特征set loss相对V1改善8/10个种子，且全部种子优于复制CT0与训练均值；
但全局MSE在10/10个种子均差于训练均值。不同指标必须同时报告，不能只展示有利项。
这些结果不支持“增加这些模块已改善复发预测”的结论，也不能证明阶段状态的临床可识别性。

正式训练使用RTX5090与BF16，冻结Swin特征未重新提取。初始129项不同软件检查通过，
随后增加11项数值复核回归检查，合计140项。真实GPU四种部分轮中断恢复均与连续训练一致。
最终审计完成80次选定/末轮检查点评分、10次训练集预处理重拟合、20个禁止源文件读取的bundle、
20次同GPU测试重放、20次跨设备逐成员概率核对、20次logistic重放和80组终点指标重算。
审计期间优化器更新为0，210个张量文件与116个冻结源码文件未改动。

首次审计因V1种子211的一个CPU/GPU raw logit相差约1.14e-5而停止。
修复仅涉及审计方法：同设备仍以原阈值核对raw logits，跨设备以原概率阈值逐成员核对；
保留固定0.5决策及CT检查，没有放宽概率或CT容差。重跑中同GPU输出完全一致，
跨设备最大概率差2.85e-6，所有分类决策一致。原失败记录保留，训练、权重与测试预测未重写。

现阶段建议保留V1作为有效对照，优先补充静态影像基线、模块消融和外部验证；
进一步在本队列上设计的实验应明确标为探索性。当前实现是已经测试的候选结构，不是CVPR创新已成立的证明。
