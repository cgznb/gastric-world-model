...
from stageworld.binary700_data import Pool
...
def score(
    ...
    if world:
        ...
        value = float(world_loss(prediction, pool.ct1[indices], valid))
        return value, {
            "ct_loss": value,
            "ct_mse": float((prediction[valid] - pool.ct1[indices][valid]).square().mean()),
        }
...
def train_phase(
    ...
    *,
    ...
    if world:
        ...
        with torch.no_grad():
            model.decoder.bias.copy_(pool.ct1[supervised].mean(0).to(device))
        fitting = supervised
    ...
    for epoch in range(first, epochs + 1):
        ...
        for rows in order.split(32):
            ...
            with torch.autocast(device.type, dtype=torch.bfloat16, enabled=device.type == "cuda"):
                if world:
                    ...
                    loss = world_loss(
                        future, pool.ct1[rows].to(device), pool.ct1_valid[rows].to(device)
                    )
                else:
                    logits = model(
                        x[rows].to(device),
        ...
        payload = {
            "artifact_id": new_artifact_id("binary700-checkpoint"),
            "contract": contract,
