...
from stageworld.binary700_inference import predict_bundle
from stageworld.binary700_spec import Candidate
from stageworld.binary700_statistics import logistic_anchor
from stageworld.binary700_training import build_model, score, train_phase
...
def verify_recovery(pool: Pool, root: Path) -> dict:
    ...
    for family in ("world", "generated"):
        ...
        parent = root / "world/17/fold-0/inner/selected.pt" if family == "generated" else None
...
def verify_study(pool: Pool, root: Path) -> dict:
    ...
    for path in sorted(root.glob("fits/*/*/fold-*/predictions.pt")):
        ...
        def guarded_open(file: Any, *args: Any, **kwargs: Any) -> Any:
            if any(text in str(file) for text in ("pool.pt", "tokens.pt", "bindings", ".xlsx")):
    ...
    if (phase_count, bundle_count) != ((18, 13) if smoke else (270, 145)):
